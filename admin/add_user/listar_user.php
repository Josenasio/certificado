...
// Buscar usuários do tipo 'Secretária' e 'Confirmação'
...
if (isset($_POST['edit_id'])) {
    ...
    $check_stmt->bind_param('si', $email, $edit_id);
    $check_stmt->execute();
    $check_stmt->store_result();
    if ($check_stmt->num_rows > 0) {
         echo "<script>alert('O email já está em uso. Por favor, escolha outro.'); window.location.href='listar_user.php';</script>";
         exit;
    }

    if (empty($senha_input)) {
         echo "<script>alert('Por favor, insira a nova senha.'); window.location.href='listar_user.php';</script>";
         exit;
    }
...
<body>
                    ...
                    <?php while ($usuario = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editModal" data-id="<?php echo $usuario['id']; ?>" data-nome="<?php echo htmlspecialchars($usuario['nome']); ?>" data-email="<?php echo htmlspecialchars($usuario['email']); ?>">
                                    <ion-icon name="create-outline"></ion-icon>
                                </button>
                                <a href="listar_user.php?delete_id=<?php echo $usuario['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Tem certeza que deseja excluir?')">
    ...
    <script>
        ...
        editModal.addEventListener('show.bs.modal', function (event) {
            ...
            modalSenha.value = ''; // Limpa a senha para que o usuário a preencha
        });
    </script>
</body>
</html>
